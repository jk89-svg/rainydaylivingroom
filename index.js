...
:root{
  ...
  --accent-mid:#0E7C86;
  --accent-dark:#114A38;
  --accent-darker:#0A3327;
  --accent-light:#2DA89A;
  --accent-subtle:#DCF3EC;
  --accent-text:#146B5C;
  --accent-you:#0E7C86;
}
body.dark{
...
.skin-arrow{background:#8B1515;color:#fff;border:none;border-radius:5px;font-size:.95rem;width:22px;height:22px;cursor:pointer;display:flex;align-items:center;justify-content:center;font-weight:900;flex-shrink:0;}
.skin-arrow:hover{background:#5A0000;}
...
.feat-arrow{background:#8B1515;color:#fff;border:none;border-radius:5px;font-size:.9rem;width:22px;height:22px;cursor:pointer;display:flex;align-items:center;justify-content:center;font-weight:900;flex-shrink:0;}
.feat-arrow:hover{background:#5A0000;}
...
.btn-play{background:linear-gradient(135deg,var(--accent),var(--accent-mid));color:#fff;text-shadow:1px 1px 0 rgba(0,0,0,.2);box-shadow:0 3px 12px rgba(0,0,0,.28);}
...
.code-btn{background:linear-gradient(135deg,var(--accent-dark),var(--accent));color:#fff;border:none;border-radius:7px;font-family:'Fredoka One',cursive;font-size:.7rem;padding:5px 9px;cursor:pointer;white-space:nowrap;}
.code-btn:hover{background:var(--accent-darker);}
...
.dm-slider{position:absolute;inset:0;background:#ccc;border-radius:18px;cursor:pointer;transition:background .2s;}
...
input:checked+.dm-slider{background:#555;}
...
#playerList{flex:1;overflow-y:auto;padding:0;}
#playerList::-webkit-scrollbar{width:3px;}#playerList::-webkit-scrollbar-thumb{background:var(--accent-light);border-radius:2px;}

/* Zebra striping for player list */
.p-entry{display:flex;align-items:center;gap:4px;padding:5px 6px;cursor:pointer;transition:opacity .12s;}
.p-entry:nth-child(odd){background-color:#ffffff;}
.p-entry:nth-child(even){background-color:#eeeeee;}
body.dark .p-entry:nth-child(odd){background-color:#000000;}
body.dark .p-entry:nth-child(even){background-color:#333333;}
.p-entry:hover{opacity:.7;}
.p-entry.muted{opacity:.4;}
...
.lc-no{background:var(--accent);color:#fff;border:none;border-radius:8px;font-family:'Fredoka One',cursive;font-size:.88rem;padding:7px 20px;cursor:pointer;}
...
body.dark .sys-vote{color:#5CD6C0;}
...
#chatSend{background:var(--accent);color:#fff;border:none;border-radius:7px;font-size:.76rem;padding:4px 8px;cursor:pointer;font-weight:700;flex-shrink:0;}
#chatSend:hover{background:var(--accent-dark);}
    ...
    <div class="home-col col-mid">
      ...
      <div class="card">
        ...
        <div style="text-align:center; margin-top:8px;">
          <span style="font-size:.62rem;font-weight:700;color:var(--accent-dark);">👥 <span id="totalPlayersCount">0</span> players active right now</span>
        </div>
      </div>
    </div>
...
<div id="kickNotice" style="display:none">
  <div class="kick-box">
    ...
    <button onclick="dismissKick()">Back to Home</button>
  </div>
</div>
...
(function(){
  ...
  const wrap=document.getElementById('livingRoom');
  if(wrap) new ResizeObserver(()=>sizeRoom()).observe(wrap);
})();
...
function startCatMeows(){
  stopCatMeows();
  const next=()=>{
    _catMeowTimer=setTimeout(()=>{ playCatMeow(); next(); },15000+Math.random()*35000);
  };
  next();
}
...
// Initialize a persistent device ID to enforce kicks across refreshes
let myDeviceId = localStorage.getItem('cg_did');
if(!myDeviceId){ 
  myDeviceId = Math.random().toString(36).substring(2) + Date.now().toString(36); 
  localStorage.setItem('cg_did', myDeviceId); 
}
...
function buildRoom(){
  ...
  svg.innerHTML=\`
<defs></defs>
<rect x="0" y="316" width="640" height="84" fill="#7A5A10"/>
<line x1="0" y1="330" x2="640" y2="330" stroke="#5A3F08" stroke-width="1" opacity=".35"/>
<line x1="0" y1="348" x2="640" y2="348" stroke="#5A3F08" stroke-width="1" opacity=".25"/>
<rect x="0" y="0" width="640" height="320" fill="#AF69EF"/>
<rect x="0" y="310" width="640" height="10" fill="#8850C4"/>
...
<ellipse cx="307" cy="326" rx="210" ry="22" fill="#B8860B"/>
<ellipse cx="307" cy="326" rx="196" ry="17" fill="#C89610"/>
<ellipse cx="307" cy="326" rx="175" ry="12" fill="#D8A610" opacity=".8"/>
<rect x="38" y="250" width="100" height="50" rx="9" fill="#F58025" stroke="#8B3A00" stroke-width="2.5"/>
<rect x="34" y="278" width="108" height="40" rx="7" fill="#F9A055" stroke="#8B3A00" stroke-width="2"/>
<rect x="30" y="260" width="13" height="53" rx="6" fill="#D06A10" stroke="#8B3A00" stroke-width="1.5"/>
<rect x="133" y="260" width="13" height="53" rx="6" fill="#D06A10" stroke="#8B3A00" stroke-width="1.5"/>
<line x1="87" y1="280" x2="87" y2="316" stroke="#8B3A00" stroke-width="1.5" opacity=".45"/>
<rect x="154" y="242" width="308" height="56" rx="11" fill="#F58025" stroke="#8B3A00" stroke-width="2.5"/>
<rect x="150" y="272" width="316" height="46" rx="9" fill="#F9A055" stroke="#8B3A00" stroke-width="2"/>
<rect x="143" y="252" width="15" height="62" rx="7" fill="#D06A10" stroke="#8B3A00" stroke-width="1.5"/>
<rect x="458" y="252" width="15" height="62" rx="7" fill="#D06A10" stroke="#8B3A00" stroke-width="1.5"/>
<line x1="229" y1="274" x2="229" y2="316" stroke="#8B3A00" stroke-width="1.5" opacity=".45"/>
<line x1="308" y1="274" x2="308" y2="316" stroke="#8B3A00" stroke-width="1.5" opacity=".45"/>
<line x1="387" y1="274" x2="387" y2="316" stroke="#8B3A00" stroke-width="1.5" opacity=".45"/>
<line x1="229" y1="244" x2="229" y2="270" stroke="#8B3A00" stroke-width="1" opacity=".3"/>
<line x1="308" y1="244" x2="308" y2="270" stroke="#8B3A00" stroke-width="1" opacity=".3"/>
<line x1="387" y1="244" x2="387" y2="270" stroke="#8B3A00" stroke-width="1" opacity=".3"/>
<rect x="504" y="250" width="100" height="50" rx="9" fill="#F58025" stroke="#8B3A00" stroke-width="2.5"/>
<rect x="500" y="278" width="108" height="40" rx="7" fill="#F9A055" stroke="#8B3A00" stroke-width="2"/>
<rect x="496" y="260" width="13" height="53" rx="6" fill="#D06A10" stroke="#8B3A00" stroke-width="1.5"/>
<rect x="599" y="260" width="13" height="53" rx="6" fill="#D06A10" stroke="#8B3A00" stroke-width="1.5"/>
<line x1="551" y1="280" x2="551" y2="316" stroke="#8B3A00" stroke-width="1.5" opacity=".45"/>
...
let catInt=null;
function startCat(){ startCatMeows();if(catInt)clearInterval(catInt);cat={x:150,y:SURFS[0][2],surf:0,vx:0,state:'idle',timer:60,dir:1,frame:0,ft:0,jp:0,jfx:0,jfy:0,jtx:0,jty:0,vis:true,offTimer:0};catInt=setInterval(()=>{catTick();drawCat();},50);}
function stopCat(){ stopCatMeows();clearInterval(catInt);catInt=null;}
...
const VOTE_KICK_COOLDOWN_MS=180000;
...
socket.on('playerLeft',({playerId,playerName,reason,lobbyState})=>{
  ...
  let msg, cls;
  if(reason==='kicked'){msg=\`\${playerName} was kicked\`;cls='sys-kick';}
  else{msg=\`\${playerName} left\`;cls='sys-leave';}
  addMsg(X(msg),cls);
...
socket.on('kicked',({lobbyCode:kCode})=>{
  window._kickedFromCode=kCode;
  window._kickedUntil=Date.now()+(10*60*1000);
  leaveGame(true);
  ...
  document.getElementById('kickMsg').textContent='You were kicked from this lobby. You cannot rejoin it for 10 minutes. You can still join or create other lobbies.';
  kn.style.display='flex';
  alert('You were kicked from this lobby.');
  clearTimeout(window._kickTimer);
  window._kickTimer=setTimeout(()=>{kn.style.display='none';},5000);
});
...
socket.on('connect',()=>{
  ...
  document.getElementById('kickNotice').style.display='none';
  clearTimeout(window._kickTimer);
});
...
function playRandom(){
  ...
  socket.emit('joinRandom',{name:document.getElementById('nameInp').value,avatar:{...AV},deviceId:myDeviceId});
}
function playCode(){
  const nameVal=document.getElementById('nameInp').value;
  const chk=validateUsername(nameVal);
  ...
  socket.emit('joinByCode',{code,name:document.getElementById('nameInp').value,avatar:{...AV},deviceId:myDeviceId});
}
...
function leaveGame(skipSocket){
  ...
  Object.values(bubbles).forEach(b=>{clearTimeout(b.t);b.el.remove();});bubbles={};
  stopAmbience();stopCat();
  if(rAF){cancelAnimationFrame(rAF);rAF=null;}
  const rc=document.getElementById('rainCanvas');
  if(rc){const rctx=rc.getContext('2d'); if(rctx) rctx.clearRect(0,0,rc.width,rc.height);}
}
function showGame(){
  document.getElementById('kickNotice').style.display='none';
  clearTimeout(window._kickTimer);
  ...
  sizeRoom();
  setTimeout(()=>{sizeRoom();initWeather();animFire();startAmbience();startCat();},80);
}
function showHome(){
  document.getElementById('homePage').style.display='flex';
  document.getElementById('gamePage').style.display='none';
  ...
  list.forEach(l=>{
    ...
    if(!isFull){
      div.onclick=()=>{
        ...
        try{getAC();}catch(e){}saveP();
        socket.emit('joinByCode',{code:l.code,name:document.getElementById('nameInp').value,avatar:{...AV},deviceId:myDeviceId});
      };
    }
    el.appendChild(div);
  });
...
function isBanned(deviceId,code){
  const b=kickBans[deviceId];if(!b||!b[code])return false;
  if(Date.now()>b[code]){delete b[code];return false;}
  return true;
}
function banLeft(deviceId,code){ return Math.max(0,(kickBans[deviceId]?.[code]||0)-Date.now()); }
...
const USERNAME_BAD = ['nigger','nigga','nigg','niga','chink','gook','spic','spick','beaner','kike','towelhead','raghead','paki','jap','redskin','injun','coon','sambo','darkie','gringo','wop','polack','faggot','fagot','dyke','tranny','shemale','cunt','whore','slut','thot','twat','porn','xxx','penis','vagina','cock','dick','pussy','tits','cum','rape','molest','pedophile','pedo','loli','kys','neonazi','kkk','whitepower','heroin','cocaine','fentanyl'];
function normU(s){return s.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/[@4]/g,'a').replace(/[3]/g,'e').replace(/[1!|]/g,'i').replace(/[05]/g,'o').replace(/[$5]/g,'s').replace(/\s+/g,'').replace(/[^a-z0-9]/g,'').replace(/(.)\1+/g,'$1');}
function isUsernameBad(name){
  if(!name||name.trim().length<2) return 'Username too short';
  const s=normU(name);
  for(const t of USERNAME_BAD){const n=t.replace(/[^a-z0-9]/g,'');if(n&&s.includes(n))return 'Username contains inappropriate content';}
  return null;
}
...
io.on('connection',(socket)=>{
  ...
  socket.on('joinRandom',({name,avatar,deviceId})=>doJoin(findAvailableLobby(deviceId),name,avatar,deviceId));

  socket.on('joinByCode',({code,name,avatar,deviceId})=>{
    ...
    if(isBanned(deviceId,uCode)){
      const mins=Math.ceil(banLeft(deviceId,uCode)/60000);
      socket.emit('joinError',{message:`You were kicked. Wait ${mins} min${mins!==1?'s':''} to rejoin.`});return;
    }
    doJoin(lobby,name,avatar,deviceId);
  });

  function doJoin(lobby,name,avatar,deviceId){
    if(isBanned(deviceId,lobby.code)){
      const mins=Math.ceil(banLeft(deviceId,lobby.code)/60000);
      socket.emit('joinError',{message:`You were kicked. Wait ${mins} min${mins!==1?'s':''} to rejoin.`});return;
    }
    ...
    curLobby=lobby; curPlayer={id:socket.id,name:cleanName,avatar:avatar||{},seat,deviceId};
  ...
  socket.on('voteKick',({targetId})=>{
    ...
    let needed;
    if(playerCount<=2) needed=2;         
    else if(playerCount<=4) needed=2;
    else needed=Math.ceil((playerCount-1)/2);
    ...
    if(voteCount>=needed){
      const tgt=lobby.players[targetId];
      if(!kickBans[tgt.deviceId]) kickBans[tgt.deviceId]={};
      kickBans[tgt.deviceId][lobby.code]=Date.now()+KICK_BAN_MS;

...
.home-logo-img{display:block;width:40%;max-width:340px;min-width:150px;height:auto;margin:4px 0 0;cursor:pointer;}
.home-topbar{display:flex;align-items:center;justify-content:center;gap:10px;margin-bottom:10px;flex-shrink:0;width:100%;max-width:920px;}
...
/* 3-column home layout. The middle column is sized naturally by its own
   content (never squeezed) — the rules/legal and lobbies columns then
   stretch to match that same height and scroll their own content
   internally, so nobody ever has to scroll the outer page to see them. */
...
.col-mid{flex:1;max-width:380px;margin:0 auto;}
...
.feat-arrow{background:#1E9E4A;color:#fff;border:none;border-radius:5px;font-size:.85rem;width:20px;height:20px;cursor:pointer;display:flex;align-items:center;justify-content:center;font-weight:900;flex-shrink:0;}
...
#gameLeftCol{position:relative;flex-shrink:0;}
.ingame-logo-img{position:absolute;left:0;bottom:100%;margin-bottom:6px;display:block;width:150px;max-width:100%;height:auto;cursor:pointer;}
...
@media(max-width:600px){
  ...
  .ingame-logo-img{width:92px!important;}
}
@media(max-width:900px){
  ...
  .ingame-logo-img{width:125px;}
  .p-name{font-size:.66rem;}
  .panel-title,.chat-title{font-size:.68rem;}
}
@media(max-width:700px){
  /* Stack columns vertically. Each card is sized by its own content here
     (normal scrolling page) — that's standard, safe mobile behavior and
     avoids ever squeezing the avatar/buttons into too small a space. */
...
<div id="homePage">
  ...
  <div class="home-3col">
    ...
    <div class="home-col col-left">
      <div class="card" id="rulesCard" style="display:flex;flex-direction:column;overflow:hidden;">
        <h3>📋 Rules & Legal</h3>
    ...
    <div class="home-col col-right">
      <div class="card" id="lobbiesCard" style="display:flex;flex-direction:column;overflow:hidden;">
        <h3>🏠 All Lobbies</h3>
...
// ── Keep the Rules/Legal and Lobbies panels the same height as the middle
//    Profile+Play card (desktop/tablet 3-column layout only), so their
//    own content scrolls inside the panel instead of the whole page ever
//    needing to scroll. Done in JS (reading the real rendered height)
//    rather than pure CSS stretch, since that's guaranteed identical on
//    every browser instead of depending on flexbox edge-case behavior.
function syncColumnHeights(){
  const midCard=document.getElementById('midCard');
  const rulesCard=document.getElementById('rulesCard');
  const lobbiesCard=document.getElementById('lobbiesCard');
  if(!midCard||!rulesCard||!lobbiesCard)return;
  if(window.innerWidth<=700){
    // Stacked mobile layout: let every card size to its own content and
    // let the page scroll normally — that's standard mobile behavior.
    rulesCard.style.height='';
    lobbiesCard.style.height='';
    return;
  }
  rulesCard.style.height='';
  lobbiesCard.style.height='';
  const h=midCard.offsetHeight;
  if(h>0){
    rulesCard.style.height=h+'px';
    lobbiesCard.style.height=h+'px';
  }
}
window.addEventListener('resize',syncColumnHeights);
window.addEventListener('orientationchange',syncColumnHeights);
window.addEventListener('load',syncColumnHeights);
setTimeout(syncColumnHeights,0);
setTimeout(syncColumnHeights,150);
